...
from .textmask import refine_mask, refine_undetected_mask, REFINEMASK_INPAINT
...
def load_model(cuda: bool):
    ...
    if cuda:
        model = TextDetector(model_path='./textblockdetector/model/comictextdetector.pt', device=device, act='leaky')
        model.net.cuda()
    else:
        model = TextDetector(model_path='./textblockdetector/model/comictextdetector.pt.onnx', device=device,
                             act='leaky', input_size=1024)
    DEFAULT_MODEL = model


def dispatch(img: np.ndarray):
    global DEFAULT_MODEL
    if DEFAULT_MODEL is None:
        load_model(torch.cuda.is_available())
    return DEFAULT_MODEL(img, refine_mode=REFINEMASK_INPAINT, keep_undetected_mask=False, bgr2rgb=True)
